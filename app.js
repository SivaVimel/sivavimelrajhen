...
const BREVITY_INSTRUCTION =
  "Response style: keep it to a single short paragraph (3–5 sentences max). " +
  "Conversational, direct, in Siva's voice. No long stories, no bullet lists, " +
  "no headings — unless the user explicitly asks for detail.";
...
function buildContextualMessage(userMessage) {
  ...
  const parts = [];
  if (recent.length) {
    parts.push("Previous conversation (for context — do not repeat verbatim):");
    recent.forEach((m) => {
  ...
  parts.push("");
  parts.push(`[Instruction] ${BREVITY_INSTRUCTION}`);
